...
Mongolia is an M1 metadata and synthetic-runtime country pack connected to the shared Postal Context API and AGID spatial index. It preserves two distinct CRC concepts: a five-digit postal zone and a nine-digit unified code, canonically written NNNNN-NNNN, that may be assigned to a building.
...
2. CRC's MNS 6775:2019 explanation distinguishes the five-digit zone from a nine-digit building-assigned code. The four-digit extension is not itself a polygon or building footprint.
