...
## Saudi Arabia source boundary

The Saudi Arabia repository owns pinned five-digit SPL National Address assignments and permitted public address fields, not a claim that the postcode is inherently a polygon. SPL BuildingNumber, AdditionalNumber, Short Address, opaque PKAddressID and an API point remain typed address evidence. An undocumented or null PolygonString, administrative area, road buffer, REGA plot, building, centroid, Voronoi cell, interpolation or model cannot become official postal geometry. A rights-cleared membership surface derived from pinned addresses is explicitly derived, uncertainty-bearing, non-canonical and versioned.

Building output requires a separately licensed authoritative feature and a source-defined stable relation, common identifier or reviewed crosswalk to the exact SPL address. GEOSA theme governance is not feature data, and REGA viewer or legal access is not vector redistribution permission. Source-specific SPL terms, retention/removal duties, SANSRS/CRS, validity, coverage, schema and digest are pinned. Public output excludes account, contact, proof, resident, occupant, owner, rightsholder, title, value, transaction, tax and query-history data. See [the Saudi Arabia runtime contract](postal-context-saudi-arabia-runtime.md).

## Oman source boundary

The Oman repository owns pinned three-digit post-office routing-code records and permitted office points, not a claim that the code is inherently a surface. The UPU country sheet establishes post-office/region coding and P.O.-box line order. An Oman Post locator point, locality centroid, governorate or wilayat, drive-time area, nearest-office partition, Voronoi cell, interpolation or model cannot become official postal geometry. Any permitted generated service surface is explicitly derived, uncertainty-bearing, non-canonical and versioned.

P.O. box, physical address, building and recipient remain separate. Exact building display requires a separately licensed feature and a source-defined stable relation, common identifier or reviewed crosswalk to the civic address. Gov.om service metadata is not an address register; NCSI portal queryability and open-data policy do not replace the exact layer licence; NSGIA governance and ONGD17 metadata are not reusable postal or building features. Public output excludes subscriber, recipient, contact, correspondence, shipment, resident, occupant, owner, title, right and query data. See [the Oman runtime contract](postal-context-oman-runtime.md).
