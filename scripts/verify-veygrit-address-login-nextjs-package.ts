...
const GATE = 'verify-veygrit-address-login-nextjs-package';
const ROOT = process.cwd();
const PACKAGE_DIR = resolve('sdk/veygrit-address-login-nextjs');
const PACKAGE_NAME = '@veygrit/address-login-nextjs';
const PACKAGE_VERSION = '0.1.0';
const EXPECTED_FILES_ALLOWLIST = ['dist', 'examples', 'README.md'] as const;
const EXPECTED_PACK_FILES = [
  'README.md',
  'dist/examples/app-router/friend-delivery/approval/route.d.ts',
  'dist/examples/app-router/friend-delivery/approval/route.js',
  'dist/examples/app-router/friend-delivery/request/route.d.ts',
  'dist/examples/app-router/friend-delivery/request/route.js',
  'dist/examples/app-router/vey-id/guest-checkout/route.d.ts',
  'dist/examples/app-router/vey-id/guest-checkout/route.js',
  'dist/examples/app-router/vey-id/revoke/route.d.ts',
  'dist/examples/app-router/vey-id/revoke/route.js',
  'dist/examples/app-router/vey-id/token/route.d.ts',
  'dist/examples/app-router/vey-id/token/route.js',
  'dist/src/index.d.ts',
  'dist/src/index.js',
...
const REQUIRED_README_ANCHORS = [
  'does not resolve raw addresses',
  'does not handle witness, private-key, proof-secret, or production credential material',
  'do not send production traffic accidentally',
  'raw address, recipient, provider token, provider raw profile, witness, private-key, proof-secret, carrier-key, or production-credential',
...
const HIGH_CONFIDENCE_SECRET_PATTERNS = [
  ...
  /VEYGRIT_WEBHOOK_SIGNING_SECRET\s*=/,
...
function exportTarget(exportsMap: unknown, key: string): ExportTarget {
  if (!isRecord(exportsMap) || !isRecord(exportsMap[key])) return {};
...
function validatePackageMetadata(packageJson: PackageJson, errors: string[]) {
  ...
  const rootExport = exportTarget(packageJson.exports, '.');
  assertEqual(rootExport.types, './dist/src/index.d.ts', 'package.exports.root.types', errors);
  assertEqual(rootExport.import, './dist/src/index.js', 'package.exports.root.import', errors);

  const serverExport = exportTarget(packageJson.exports, './server');
  assertEqual(serverExport.types, './dist/src/server.d.ts', 'package.exports.server.types', errors);
  assertEqual(serverExport.import, './dist/src/server.js', 'package.exports.server.import', errors);
}
...
function validatePackSummary(packSummary: PackSummary | undefined, errors: string[]) {
  ...
  assertEqual(packSummary.filename, 'veygrit-address-login-nextjs-0.1.0.tgz', 'pack.filename', errors);
  ...
  const tarballPath = join(PACKAGE_DIR, 'veygrit-address-login-nextjs-0.1.0.tgz');
