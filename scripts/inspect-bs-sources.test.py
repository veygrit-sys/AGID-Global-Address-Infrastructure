...
MODULE = runpy.run_path(str(Path(__file__).with_name("inspect-bs-sources.py")))
inspect_source_dir = MODULE["inspect_source_dir"]
digest = MODULE["digest"]


HTML_MARKERS = {
    "bahamas-transport-home.html": ["Post Office Department oversight"],
...
def synthetic_expected(tmpdir):
    ...
    for name in ["upu-bhs-addressing.pdf", "upu-general-addressing.pdf", "bahamas-postal-rate-book.pdf"]:
        body = b"%PDF-fixture"
        (root / name).write_bytes(body)
        specs.append({
            "file": name,
...
class SourceInspectorTests(unittest.TestCase):
    ...
    def test_exact_complete_set_is_accepted_without_rows_or_promotion(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(report["exactBodiesByteAndSha256Bound"], 6)
    ...
    def test_changed_body_fails_closed(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (Path(tmpdir) / "bahamas-transport-home.html").write_text("changed", encoding="utf-8")
    ...
    def test_missing_extra_and_renamed_sources_fail_closed(self):
        for mutation in ["missing", "extra", "renamed"]:
            with self.subTest(mutation=mutation), tempfile.TemporaryDirectory() as tmpdir:
                expected = synthetic_expected(tmpdir)
                path = Path(tmpdir) / "bahamas-transport-home.html"
                if mutation == "missing":
                    path.unlink()
                elif mutation == "extra":
    ...
    def test_po_box_and_office_abbreviations_never_promote(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            report = inspect_source_dir(tmpdir, synthetic_expected(tmpdir))
            self.assertTrue(report["upuBahamasSaysNoPostcodesOrHomeDelivery"])
            self.assertFalse(report["poBoxOrPostOfficeAbbreviationTreatedAsPostcode"])
