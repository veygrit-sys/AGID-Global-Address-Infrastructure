...
MODULE=runpy.run_path(str(Path(__file__).with_name('inspect-postal-context-cd-sources.py')))
...
def fixture(root):
 ...
 return [{'file':path.name,'url':'https://official.invalid/x','bytes':len(body),'sha256':digest(body),'kind':'json','candidate':True}]

class Tests(unittest.TestCase):
 def test_missing_source_fails_closed(self):
  with tempfile.TemporaryDirectory() as tmp:
   with self.assertRaisesRegex(ValueError,'source-set-mismatch'): inspect_source_dir(tmp,fixture(tmp)+[{'file':'missing.json','url':'x','bytes':1,'sha256':'0'*64,'kind':'json'}])
 ...
 def test_marker_drift_fails_closed(self):
  with tempfile.TemporaryDirectory() as tmp:
   expected=fixture(tmp); expected=copy.deepcopy(expected); expected[0]['kind']='text'; expected[0]['markers']=['absent']
