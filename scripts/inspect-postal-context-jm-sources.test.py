...
MODULE = runpy.run_path(str(Path(__file__).with_name("inspect-postal-context-jm-sources.py")))
        ...
        for mutation in ["signature", "pages", "marker"]:
            with self.subTest(mutation=mutation), tempfile.TemporaryDirectory() as tmpdir:
                ...
                if mutation == "signature":
                    body = b"not-pdf"; path = Path(tmpdir) / "fixture.pdf"; path.write_bytes(body)
                    expected[0].update(bytes=len(body), sha256=digest(body)); message = "invalid-pdf"
