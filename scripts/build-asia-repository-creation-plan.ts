...
function loadAsiaPlacement() {
  ...
  const asia = placement.continents.find(continent => continent.id === 'asia');
  if (!asia) throw new Error('Missing asia placement in agid-repository-placement.json');
  return asia;
}
...
export function buildAsiaRepositoryCreationPlan() {
  const asia = loadAsiaPlacement();
  const repositories: CreationPlanRepo[] = [
    {
      repository: asia.repository,
      ...
      description: 'AGID Asia index for Caucasus, East Asia, Southeast Asia, South Asia, Central Asia, Middle East, territories, and staged country packs.',
    },
  ];

  for (const region of asia.regions) {
    repositories.push({
      ...
      description: `AGID ${titleFromId(region.id)} index for Asia country and territory repository coordination.`,
    });
  ...
  const summary = {
    continent: 'asia',
    rootRepository: asia.repository,
  ...
  return {
    ...
    version: 'agid-asia-repository-creation-plan-v1',
    owner: 'dawnportinfo-design',
    policy: {
      physicalCreation: 'Create the Asia continent and region indexes first. Country parents and child repositories remain planned until maintainers and datasets are ready.',
      ...
      canonicalRegions: 'Keep AM, AZ, and GE canonical under Asia/Caucasus; political claims are not encoded as ownership decisions.',
...
export function writeAsiaRepositoryCreationPlan() {
  const plan = buildAsiaRepositoryCreationPlan();
  const outputPath = path.resolve('data/global_entities/agid-asia-repository-creation-plan.json');
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const { outputPath, plan } = writeAsiaRepositoryCreationPlan();
