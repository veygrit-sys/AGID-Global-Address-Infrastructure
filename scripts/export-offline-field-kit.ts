...
async function main() {
  const generatedAt = process.env.OFFLINE_FIELD_KIT_GENERATED_AT || DEFAULT_GENERATED_AT;
  const kit = buildOfflineFieldKit({ generatedAt });
  const validation = validateOfflineFieldKit(kit);

  if (!validation.valid) {
    throw new Error(`Offline Field Kit is invalid: ${validation.errors.join(', ')}`);
  }

  const outDir = join(process.cwd(), 'data', 'offline_field_kit');
  await mkdir(outDir, { recursive: true });
  await writeJson(outDir, 'manifest.json', kit.manifest);
  await writeJson(outDir, 'offline-field-kit.json', kit);
  await writeJson(outDir, 'runbooks.json', kit.runbooks);
  ...
  console.log(`Offline Field Kit exported to ${outDir}`);
  console.log(`runbooks=${kit.manifest.counts.runbooks}`);
