...
MODULE = runpy.run_path(str(Path(__file__).with_name("inspect-postal-context-sr-sources.py")))
...
def synthetic_expected(tmpdir):
    ...
    return [
        {"file":"fixture.pdf","url":"https://official.invalid/fixture.pdf","bytes":pdf_path.stat().st_size,"sha256":digest(pdf_path.read_bytes()),"kind":"pdf","pages":1,"page_markers":{0:["marker"]}},
        {"file":"fixture.html","url":"https://official.invalid/fixture.html","bytes":html_path.stat().st_size,"sha256":digest(html_path.read_bytes()),"kind":"html","markers":["marker"]},
    ]
...
class SourceInspectorTests(unittest.TestCase):
    ...
    def test_wrong_html_marker_fails_closed(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            expected = synthetic_expected(tmpdir); expected[1] = copy.deepcopy(expected[1]); expected[1]["markers"] = ["absent"]
