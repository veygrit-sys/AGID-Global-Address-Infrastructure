...
MODULE = runpy.run_path(str(Path(__file__).with_name("inspect-postal-context-gd-sources.py")))
...
def synthetic_expected(tmpdir):
    ...
    return [
        {
            "file": "fixture.pdf",
            "url": "https://official.invalid/fixture.pdf",
            "bytes": pdf_path.stat().st_size,
            "sha256": digest(pdf_path.read_bytes()),
            "kind": "pdf",
            "pages": 1,
            "page_markers": {0: ["marker"]},
            "forbidden": [r"postcode"],
        },
        {
            "file": "fixture.html",
            "url": "https://official.invalid/fixture.html",
            "bytes": html_path.stat().st_size,
            "sha256": digest(html_path.read_bytes()),
            "kind": "html",
            "markers": ["marker"],
            "forbidden": [r"postcode"],
        },
...
class SourceInspectorTests(unittest.TestCase):
    ...
    def test_wrong_html_marker_and_unexpected_postcode_fail_closed(self):
        for mutation in ["marker", "postcode"]:
            with self.subTest(mutation=mutation), tempfile.TemporaryDirectory() as tmpdir:
                expected = synthetic_expected(tmpdir)
                expected[1] = copy.deepcopy(expected[1])
                if mutation == "marker":
                    expected[1]["markers"] = ["absent"]
                    message = "missing-html-marker"
                else:
                    body = b"<html>marker postcode</html>"
                    path = Path(tmpdir) / "fixture.html"
                    path.write_bytes(body)
                    expected[1].update(bytes=len(body), sha256=digest(body))
                    message = "unexpected-postcode-marker"
                with self.assertRaisesRegex(ValueError, message):
                    inspect_source_dir(tmpdir, expected)

    def test_wrong_pdf_signature_page_count_marker_and_postcode_fail_closed(self):
        for mutation in ["signature", "pages", "marker", "postcode"]:
            with self.subTest(mutation=mutation), tempfile.TemporaryDirectory() as tmpdir:
                expected = synthetic_expected(tmpdir)
                ...
                if mutation == "signature":
                    ...
                    MODULE["pdfplumber"].open = lambda _path: type(
                        "PostcodePdf",
                        (),
                        {
                            "pages": [type("Page", (), {"extract_text": lambda self: "marker postcode"})()],
