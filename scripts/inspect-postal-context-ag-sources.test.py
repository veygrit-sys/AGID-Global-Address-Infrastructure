...
MODULE = runpy.run_path(str(Path(__file__).with_name("inspect-postal-context-ag-sources.py")))
...
class SourceInspectorTests(unittest.TestCase):
    ...
    def test_exact_complete_set_is_accepted_without_rows_or_promotion(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            report = inspect_source_dir(tmpdir, expected)
            self.assertEqual(report["exactBodiesByteAndSha256Bound"], 2)
            self.assertEqual(report["currentPostalIdentifierRowsValidated"], 0)
    ...
    def test_changed_body_fails_closed(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            expected = synthetic_expected(tmpdir)
            (Path(tmpdir) / "fixture.html").write_text("<html>changed</html>", encoding="utf-8")
