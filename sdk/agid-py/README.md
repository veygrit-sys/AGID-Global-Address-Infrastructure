...
| Runtime | Python >=3.10 |
| Primary use | Data pipelines, GIS notebooks, and backend automation |
...
Before publishing this SDK, run `python -m pytest` and confirm every parity vector in `agid-parity-vectors.json` matches the canonical `agid-spec/test-vectors.json`.
