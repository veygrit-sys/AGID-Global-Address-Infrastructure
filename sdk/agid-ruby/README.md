...
| Runtime | Ruby >=3.0 |
| Primary use | Rails and scripting integrations |
| Test command | `bundle exec ruby test/test_parity.rb` |
...
Before publishing this SDK, run `bundle exec ruby test/test_parity.rb` and confirm every parity vector in `agid-parity-vectors.json` matches the canonical `agid-spec/test-vectors.json`.
