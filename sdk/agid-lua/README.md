...
| Runtime | Lua >=5.4 |
| Primary use | Embedded scripting and lightweight devices |
| Test command | `lua test/parity_test.lua` |
...
Before publishing this SDK, run `lua test/parity_test.lua` and confirm every parity vector in `agid-parity-vectors.json` matches the canonical `agid-spec/test-vectors.json`.
