...
| Package manager | Pkg |
| Runtime | Julia >=1.9 |
| Primary use | Scientific computing and routing research |
...
Before publishing this SDK, run `julia --project=. -e "using Pkg; Pkg.test()"` and confirm every parity vector in `agid-parity-vectors.json` matches the canonical `agid-spec/test-vectors.json`.
