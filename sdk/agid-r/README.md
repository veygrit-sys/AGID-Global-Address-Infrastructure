...
| Runtime | R >=4.1 |
| Primary use | Statistics, research, and geospatial analysis |
...
Before publishing this SDK, run `Rscript -e "testthat::test_dir(\"tests/testthat\")"` and confirm every parity vector in `agid-parity-vectors.json` matches the canonical `agid-spec/test-vectors.json`.
