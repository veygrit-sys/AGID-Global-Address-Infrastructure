...
| Runtime | C++17 |
| Primary use | GIS engines, desktop, and native route systems |
| Test command | `c++ tests/parity.cpp src/agid.cpp -Iinclude -std=c++17 && ./a.out` |
...
Before publishing this SDK, run `c++ tests/parity.cpp src/agid.cpp -Iinclude -std=c++17 && ./a.out` and confirm every parity vector in `agid-parity-vectors.json` matches the canonical `agid-spec/test-vectors.json`.
