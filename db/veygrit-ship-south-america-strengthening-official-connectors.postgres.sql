-- Strengthen South America with direct official carrier adapters.
-- Re-runnable after the major Europe strengthening migration.
...
BEGIN
  FOR item IN
    ...
    WHERE c.contype = 'c'
      ...
      AND (
        c.conname LIKE '%_carrier_major_europe_strengthening_chk'
        OR c.conname LIKE '%_adapter_major_europe_strengthening_chk'
        OR c.conname LIKE '%_carrier_south_america_strengthening_chk'
  ...
  EXECUTE format(
    'ALTER TABLE veygrit_ship_carrier_connection ADD CONSTRAINT veygrit_ship_connection_carrier_south_america_strengthening_chk CHECK (carrier IN (%s))',
    carrier_values
  );
  EXECUTE format(
    'ALTER TABLE veygrit_ship_shipment ADD CONSTRAINT veygrit_ship_shipment_carrier_south_america_strengthening_chk CHECK (carrier IN (%s))',
    carrier_values
  );
  EXECUTE format(
    'ALTER TABLE veygrit_ship_shipment ADD CONSTRAINT veygrit_ship_shipment_adapter_south_america_strengthening_chk CHECK (adapter IN (%s))',
    adapter_values
  );
  EXECUTE format(
    'ALTER TABLE veygrit_ship_rate_quote ADD CONSTRAINT veygrit_ship_rate_quote_carrier_south_america_strengthening_chk CHECK (carrier IN (%s))',
    carrier_values
  );
  EXECUTE format(
    'ALTER TABLE veygrit_ship_rate_quote ADD CONSTRAINT veygrit_ship_rate_quote_adapter_south_america_strengthening_chk CHECK (adapter IN (%s))',
    adapter_values
  );
  EXECUTE format(
    'ALTER TABLE veygrit_ship_label ADD CONSTRAINT veygrit_ship_label_carrier_south_america_strengthening_chk CHECK (carrier IN (%s))',
    carrier_values
  );
  EXECUTE format(
    'ALTER TABLE veygrit_ship_tracking_event ADD CONSTRAINT veygrit_ship_tracking_carrier_south_america_strengthening_chk CHECK (carrier IN (%s))',
    carrier_values
  );
END;
$$;
...
ALTER TABLE veygrit_ship_shipment
  DROP CONSTRAINT IF EXISTS veygrit_ship_shipment_andreani_adapter_chk,
  DROP CONSTRAINT IF EXISTS veygrit_ship_shipment_servientrega_adapter_chk,
  DROP CONSTRAINT IF EXISTS veygrit_ship_shipment_blue_express_adapter_chk,
  ADD CONSTRAINT veygrit_ship_shipment_andreani_adapter_chk
    CHECK (
      carrier <> 'andreani'
      OR adapter IS NULL
      OR adapter = 'andreani-globallpack-v1'
    ),
  ADD CONSTRAINT veygrit_ship_shipment_servientrega_adapter_chk
    CHECK (
      carrier <> 'servientrega'
      OR adapter IS NULL
      OR adapter = 'servientrega-standard-v1'
    ),
  ADD CONSTRAINT veygrit_ship_shipment_blue_express_adapter_chk
    CHECK (
      carrier <> 'blue_express'
      OR adapter IS NULL
      OR adapter = 'blue-express-contract-v1'
    );
