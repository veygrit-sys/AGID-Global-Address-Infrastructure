-- Strengthen South Asia with direct official and carrier-contract adapters.
-- Re-runnable after the Southeast Asia strengthening migration.
...
BEGIN
  FOR item IN
    ...
    WHERE c.contype = 'c'
      ...
      AND (
        c.conname LIKE '%_carrier_southeast_asia_strengthening_chk'
        OR c.conname LIKE '%_adapter_southeast_asia_strengthening_chk'
        OR c.conname LIKE '%_carrier_south_asia_strengthening_chk'
  ...
  EXECUTE format(
    'ALTER TABLE veygrit_ship_carrier_connection ADD CONSTRAINT veygrit_ship_connection_carrier_south_asia_strengthening_chk CHECK (carrier IN (%s))',
    carrier_values
  );
  EXECUTE format(
    'ALTER TABLE veygrit_ship_shipment ADD CONSTRAINT veygrit_ship_shipment_carrier_south_asia_strengthening_chk CHECK (carrier IN (%s))',
    carrier_values
  );
  EXECUTE format(
    'ALTER TABLE veygrit_ship_shipment ADD CONSTRAINT veygrit_ship_shipment_adapter_south_asia_strengthening_chk CHECK (adapter IN (%s))',
    adapter_values
  );
  EXECUTE format(
    'ALTER TABLE veygrit_ship_rate_quote ADD CONSTRAINT veygrit_ship_rate_quote_carrier_south_asia_strengthening_chk CHECK (carrier IN (%s))',
    carrier_values
  );
  EXECUTE format(
    'ALTER TABLE veygrit_ship_rate_quote ADD CONSTRAINT veygrit_ship_rate_quote_adapter_south_asia_strengthening_chk CHECK (adapter IN (%s))',
    adapter_values
  );
  EXECUTE format(
    'ALTER TABLE veygrit_ship_label ADD CONSTRAINT veygrit_ship_label_carrier_south_asia_strengthening_chk CHECK (carrier IN (%s))',
    carrier_values
  );
  EXECUTE format(
    'ALTER TABLE veygrit_ship_tracking_event ADD CONSTRAINT veygrit_ship_tracking_carrier_south_asia_strengthening_chk CHECK (carrier IN (%s))',
    carrier_values
  );
END;
$$;
...
ALTER TABLE veygrit_ship_shipment
  DROP CONSTRAINT IF EXISTS veygrit_ship_shipment_pathao_courier_adapter_chk,
  DROP CONSTRAINT IF EXISTS veygrit_ship_shipment_ecourier_bd_adapter_chk,
  DROP CONSTRAINT IF EXISTS veygrit_ship_shipment_leopards_courier_adapter_chk,
  DROP CONSTRAINT IF EXISTS veygrit_ship_shipment_domex_lk_adapter_chk,
  DROP CONSTRAINT IF EXISTS veygrit_ship_shipment_nepal_can_move_adapter_chk,
  ADD CONSTRAINT veygrit_ship_shipment_pathao_courier_adapter_chk
    CHECK (carrier <> 'pathao_courier' OR adapter IS NULL OR adapter = 'pathao-courier-v1'),
  ADD CONSTRAINT veygrit_ship_shipment_ecourier_bd_adapter_chk
    CHECK (carrier <> 'ecourier_bd' OR adapter IS NULL OR adapter = 'ecourier-merchant-v5.4'),
  ADD CONSTRAINT veygrit_ship_shipment_leopards_courier_adapter_chk
    CHECK (carrier <> 'leopards_courier' OR adapter IS NULL OR adapter = 'leopards-merchant-contract-v1'),
  ADD CONSTRAINT veygrit_ship_shipment_domex_lk_adapter_chk
    CHECK (carrier <> 'domex_lk' OR adapter IS NULL OR adapter = 'domex-client-contract-v1'),
  ADD CONSTRAINT veygrit_ship_shipment_nepal_can_move_adapter_chk
    CHECK (carrier <> 'nepal_can_move' OR adapter IS NULL OR adapter = 'nepal-can-move-contract-v1');
