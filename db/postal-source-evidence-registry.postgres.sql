-- Metadata-only postal source evidence registry for Postgres AGID deployments.
...
CREATE TABLE IF NOT EXISTS postal_source_candidate (
  ...
  catalog_metadata_digest TEXT NOT NULL CHECK (char_length(catalog_metadata_digest) > 0),
  raw_private_material_stored BOOLEAN NOT NULL DEFAULT FALSE CHECK (raw_private_material_stored = FALSE),
  CHECK (
    (source_version_status = 'not-recorded' AND source_version IS NULL)
    OR (source_version_status IN ('observed-unverified', 'verified-current') AND source_version IS NOT NULL AND char_length(source_version) > 0)
  ),
  CHECK (
...
CREATE TABLE IF NOT EXISTS postal_source_record (
  ...
  review_by TIMESTAMPTZ NOT NULL,
  source_digest TEXT NOT NULL CHECK (char_length(source_digest) > 0),
  terms_digest TEXT NOT NULL CHECK (char_length(terms_digest) > 0),
...
CREATE TABLE IF NOT EXISTS postal_source_quality_evidence (
  ...
  reviewed_at TIMESTAMPTZ NOT NULL,
  quality_review_by TIMESTAMPTZ NOT NULL,
  raw_private_material_stored BOOLEAN NOT NULL DEFAULT FALSE CHECK (raw_private_material_stored = FALSE),
...
CREATE TABLE IF NOT EXISTS postal_source_quality_attestation (
  ...
  signer_key_fingerprint TEXT NOT NULL CHECK (char_length(signer_key_fingerprint) > 0),
  signature_digest TEXT NOT NULL CHECK (char_length(signature_digest) > 0),
  verified_at TIMESTAMPTZ NOT NULL,
  raw_private_material_stored BOOLEAN NOT NULL DEFAULT FALSE CHECK (raw_private_material_stored = FALSE),
...
CREATE TABLE IF NOT EXISTS postal_validation_runtime_contract (
  ...
  contract_digest TEXT NOT NULL CHECK (char_length(contract_digest) > 0),
  reviewed_at TIMESTAMPTZ NOT NULL,
  runtime_review_by TIMESTAMPTZ NOT NULL,
  raw_private_material_stored BOOLEAN NOT NULL DEFAULT FALSE CHECK (raw_private_material_stored = FALSE),
...
CREATE TABLE IF NOT EXISTS postal_validation_language_policy (
  ...
  scope_id TEXT NOT NULL REFERENCES postal_source_scope(scope_id) ON DELETE CASCADE,
  language_tag TEXT NOT NULL CHECK (char_length(language_tag) >= 2),
  script_code TEXT NOT NULL CHECK (char_length(script_code) = 4),
  ...
  policy_digest TEXT NOT NULL CHECK (char_length(policy_digest) > 0),
  reviewed_at TIMESTAMPTZ NOT NULL,
  language_review_by TIMESTAMPTZ NOT NULL,
  raw_private_material_stored BOOLEAN NOT NULL DEFAULT FALSE CHECK (raw_private_material_stored = FALSE),
...
SELECT
  ...
  CASE
    WHEN scope.scope_status = 'metadata-only-approved'
      AND scope.raw_private_material_stored = FALSE
      ...
      AND source.review_by >= CURRENT_TIMESTAMP
      AND source.raw_private_material_stored = FALSE
      AND EXISTS (
        SELECT 1
        FROM postal_source_correction_evidence AS correction
        WHERE correction.source_record_id = source.source_record_id
          AND correction.raw_private_material_stored = FALSE
      )
      AND COALESCE((
        ...
        WHERE quality_evidence.source_record_id = source.source_record_id
          ...
          AND quality_evidence.quality_review_by >= CURRENT_TIMESTAMP
          AND quality_evidence.raw_private_material_stored = FALSE
          AND COALESCE((
            ...
            WHERE attestation.quality_evidence_id = quality_evidence.quality_evidence_id
              ...
              AND attestation.raw_private_material_stored = FALSE
      ...
      AND COALESCE((
        ...
        WHERE review_event.source_record_id = source.source_record_id
          AND review_event.raw_private_material_stored = FALSE
  ...
  FALSE AS delivery_claims_enabled
...
SELECT
  ...
  CASE
    WHEN scope.scope_status = 'metadata-only-approved'
      ...
      AND candidate.review_by >= CURRENT_TIMESTAMP
      AND candidate.raw_private_material_stored = FALSE
    THEN TRUE
    ELSE FALSE
...
SELECT
  ...
  CASE
    WHEN readiness.official_reference_validation_eligible = TRUE
      ...
      AND contract.runtime_review_by >= CURRENT_TIMESTAMP
      AND contract.raw_private_material_stored = FALSE
    THEN TRUE
    ELSE FALSE
...
SELECT
  ...
  CASE
    WHEN readiness.official_reference_validation_eligible = TRUE
      ...
      AND policy.language_review_by >= CURRENT_TIMESTAMP
      AND policy.raw_private_material_stored = FALSE
    THEN TRUE
    ELSE FALSE
