...
test('Montenegro Postal Context route resolves an explicitly linked UZN building with ME AGID', async () => {
  const response = await fetch(`${baseUrl}/api/postal/resolve`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-AGID-Request-ID': 'me-postal-context-test',
    },
    body: JSON.stringify({
      countryCode: 'ME',
      ...MONTENEGRO_POSTAL_CONTEXT_TEST_POINT,
  ...
  assert.equal(body.data.countryCode, 'ME');
...
test('Montenegro postcode route canonicalizes whitespace and gates derived geometry', async () => {
  const query = new URLSearchParams({
    validAt: MONTENEGRO_POSTAL_CONTEXT_TEST_INSTANT,
    geometry: 'geojson',
  });
  const response = await fetch(`${baseUrl}/api/postal/ME/${encodeURIComponent('00 000')}?${query}`, {
    headers: { 'X-AGID-Request-ID': 'me-postal-lookup-test' },
  });
  const body = await response.json() as any;
