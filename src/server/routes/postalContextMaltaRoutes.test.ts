...
test('Malta Postal Context route resolves an explicitly linked PA building with AGID', async () => {
  const response = await fetch(`${baseUrl}/api/postal/resolve`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-AGID-Request-ID': 'mt-postal-context-test',
    },
    body: JSON.stringify({
      countryCode: 'MT',
      ...MALTA_POSTAL_CONTEXT_TEST_POINT,
  ...
  assert.equal(body.data.countryCode, 'MT');
...
test('Malta postcode route canonicalizes compact input and gates derived geometry', async () => {
  const query = new URLSearchParams({
    validAt: MALTA_POSTAL_CONTEXT_TEST_INSTANT,
    geometry: 'geojson',
  });
  const response = await fetch(`${baseUrl}/api/postal/MT/zzz0000?${query}`, {
    headers: { 'X-AGID-Request-ID': 'mt-postal-lookup-test' },
  });
  const body = await response.json() as any;

  assert.equal(response.status, 200);
  assert.equal(body.ok, true);
  assert.equal(body.data.normalizedPostalCode, 'ZZZ 0000');
