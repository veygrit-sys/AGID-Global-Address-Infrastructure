...
test('Italy Postal Context route resolves source-linked building context with AGID', async () => {
  const response = await fetch(`${baseUrl}/api/postal/resolve`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-AGID-Request-ID': 'it-postal-context-test',
    },
    body: JSON.stringify({
      countryCode: 'IT',
      ...ITALY_POSTAL_CONTEXT_TEST_POINT,
  ...
  assert.equal(body.data.countryCode, 'IT');
...
test('Italy CAP route preserves leading zeroes and gates derived geometry', async () => {
  const query = new URLSearchParams({
    validAt: ITALY_POSTAL_CONTEXT_TEST_INSTANT,
    geometry: 'geojson',
  });
  const response = await fetch(`${baseUrl}/api/postal/IT/00000?${query}`, {
    headers: { 'X-AGID-Request-ID': 'it-postal-lookup-test' },
  });
  const body = await response.json() as any;
