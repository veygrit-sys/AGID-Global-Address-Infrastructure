...
const validAt = '2026-08-31T19:42:34.212Z';
let server: Server;
let baseUrl: string;
async function lookup(postalCode: string, expectedStatus = 200) {
  const response = await fetch(baseUrl + '/api/postal/CP/' + encodeURIComponent(postalCode) + '?validAt=' + encodeURIComponent(validAt) + '&geometry=geojson');
  const envelope = await response.json() as { ok: boolean; data: PostalContextLookupResponse };
  assert.equal(response.status, expectedStatus);
  return envelope.data;
}
before(async () => {
  const loaded = loadPostalContextPack(resolve('data/postal_country_packs/cp/postal-context/m2/descriptor.json'), descriptorDigest,
    { expectedCountryCode: 'CP', allowExperimental: true });
  const app = express();
...
test('real CP lookup reaches API, app conversion, fit, translucent paint, clear and re-search', async () => {
  const data = await lookup('９８７９９');
  assert.equal(data.status, 'unique');
  assert.equal(data.normalizedPostalCode, '98799');
  ...
  assert.deepEqual(postalAreaBounds(collection), [[-109.234607, 10.287154], [-109.19979, 10.31957]]);
...
test('CP invalid postcode is rejected without a fabricated area', async () => {
  const invalidResponse = await fetch(baseUrl + '/api/postal/CP/98798?validAt=' + encodeURIComponent(validAt) + '&geometry=geojson');
