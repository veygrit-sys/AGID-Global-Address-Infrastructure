...
test('Hungary Postal Context route resolves an explicitly linked building with HU AGID', async () => {
  const response = await fetch(`${baseUrl}/api/postal/resolve`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-AGID-Request-ID': 'hu-postal-context-test',
    },
    body: JSON.stringify({
      countryCode: 'HU',
      ...HUNGARY_POSTAL_CONTEXT_TEST_POINT,
  ...
  assert.equal(body.data.countryCode, 'HU');
...
test('Hungary postcode route canonicalizes whitespace and gates derived geometry', async () => {
  const query = new URLSearchParams({
    validAt: HUNGARY_POSTAL_CONTEXT_TEST_INSTANT,
    geometry: 'geojson',
  });
  const response = await fetch(`${baseUrl}/api/postal/HU/${encodeURIComponent('00 00')}?${query}`, {
    headers: { 'X-AGID-Request-ID': 'hu-postal-lookup-test' },
  });
  const body = await response.json() as any;
  ...
  assert.equal(body.data.geometries[0].source.sourceId, 'hu-synthetic-derived-postcode-surface');
});
