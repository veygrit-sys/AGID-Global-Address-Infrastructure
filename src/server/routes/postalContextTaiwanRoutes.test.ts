...
test('Taiwan Postal Context route resolves an explicitly linked NLSC building with TW AGID', async () => {
  const response = await fetch(`${baseUrl}/api/postal/resolve`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-AGID-Request-ID': 'tw-postal-context-test',
    },
    body: JSON.stringify({
      countryCode: 'TW',
      ...TAIWAN_POSTAL_CONTEXT_TEST_POINT,
  ...
  assert.equal(body.data.countryCode, 'TW');
...
test('Taiwan postcode route canonicalizes whitespace and gates derived geometry', async () => {
  const query = new URLSearchParams({
    validAt: TAIWAN_POSTAL_CONTEXT_TEST_INSTANT,
    geometry: 'geojson',
  });
  const response = await fetch(
    `${baseUrl}/api/postal/TW/${encodeURIComponent('000 000')}?${query}`,
    { headers: { 'X-AGID-Request-ID': 'tw-postal-lookup-test' } },
  );
  const body = await response.json() as any;
