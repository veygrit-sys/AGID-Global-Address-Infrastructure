...
test('Denmark Postal Context route resolves an explicitly linked GeoDanmark building with AGID', async () => {
  const response = await fetch(`${baseUrl}/api/postal/resolve`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-AGID-Request-ID': 'dk-postal-context-test',
    },
    body: JSON.stringify({
      countryCode: 'DK',
      ...DENMARK_POSTAL_CONTEXT_TEST_POINT,
  ...
  assert.equal(body.data.countryCode, 'DK');
...
test('Denmark postcode route preserves leading zeroes and gates geometry', async () => {
  const query = new URLSearchParams({
    validAt: DENMARK_POSTAL_CONTEXT_TEST_INSTANT,
    geometry: 'geojson',
  });
  const response = await fetch(`${baseUrl}/api/postal/DK/0000?${query}`, {
    headers: { 'X-AGID-Request-ID': 'dk-postal-lookup-test' },
  });
  const body = await response.json() as any;
