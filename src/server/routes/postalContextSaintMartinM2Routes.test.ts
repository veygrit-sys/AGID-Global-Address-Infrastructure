...
const validAt = '2026-09-01T12:10:26.118Z';
let server: Server;
let baseUrl: string;
async function lookup(postalCode: string, expectedStatus = 200) {
  const response = await fetch(baseUrl + '/api/postal/MF/' + encodeURIComponent(postalCode) + '?validAt=' + encodeURIComponent(validAt) + '&geometry=geojson');
  const envelope = await response.json() as { ok: boolean; data: PostalContextLookupResponse };
  assert.equal(response.status, expectedStatus);
  return envelope.data;
}
before(async () => {
  const loaded = loadPostalContextPack(resolve('data/postal_country_packs/mf/postal-context/m2/descriptor.json'), descriptorDigest,
    { expectedCountryCode: 'MF', allowExperimental: true });
  const app = express();
...
test('real MF lookup reaches API, app conversion, fit, translucent paint, clear and re-search', async () => {
  const data = await lookup('９７１５０');
  assert.equal(data.status, 'unique');
  assert.equal(data.normalizedPostalCode, '97150');
  ...
  assert.deepEqual(postalAreaBounds(collection), [[-63.15332, 18.045903], [-62.970711, 18.125195]]);
...
test('MF invalid postcode is rejected without a fabricated area', async () => {
  const invalidResponse = await fetch(baseUrl + '/api/postal/MF/97151?validAt=' + encodeURIComponent(validAt) + '&geometry=geojson');
