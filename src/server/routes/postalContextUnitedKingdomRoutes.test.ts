...
test('United Kingdom Postal Context route resolves source-linked building context with AGID', async () => {
  const response = await fetch(`${baseUrl}/api/postal/resolve`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-AGID-Request-ID': 'gb-postal-context-test',
    },
    body: JSON.stringify({
      countryCode: 'GB',
      ...UNITED_KINGDOM_POSTAL_CONTEXT_TEST_POINT,
  ...
  assert.equal(body.data.countryCode, 'GB');
...
test('United Kingdom postcode route normalizes compact input and gates derived geometry', async () => {
  const query = new URLSearchParams({
    validAt: UNITED_KINGDOM_POSTAL_CONTEXT_TEST_INSTANT,
    geometry: 'geojson',
  });
  const response = await fetch(`${baseUrl}/api/postal/GB/ZZ00ZZ?${query}`, {
    headers: { 'X-AGID-Request-ID': 'gb-postal-lookup-test' },
  });
  const body = await response.json() as any;

  assert.equal(response.status, 200);
  assert.equal(body.ok, true);
  assert.equal(body.data.normalizedPostalCode, 'ZZ0 0ZZ');
