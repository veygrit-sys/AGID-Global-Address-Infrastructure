...
test('Estonia Postal Context route resolves ADS-linked building context with AGID', async () => {
  const response = await fetch(`${baseUrl}/api/postal/resolve`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-AGID-Request-ID': 'ee-postal-context-test',
    },
    body: JSON.stringify({
      countryCode: 'EE',
      ...ESTONIA_POSTAL_CONTEXT_TEST_POINT,
  ...
  assert.equal(body.data.countryCode, 'EE');
...
test('Estonia postcode route preserves leading zeroes and gates geometry', async () => {
  const query = new URLSearchParams({
    validAt: ESTONIA_POSTAL_CONTEXT_TEST_INSTANT,
    geometry: 'geojson',
  });
  const response = await fetch(`${baseUrl}/api/postal/EE/00000?${query}`, {
    headers: { 'X-AGID-Request-ID': 'ee-postal-lookup-test' },
  });
  const body = await response.json() as any;
