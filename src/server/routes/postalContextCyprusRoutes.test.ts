...
test('Cyprus Postal Context route resolves an explicitly linked DLS building with CY AGID', async () => {
  const response = await fetch(`${baseUrl}/api/postal/resolve`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-AGID-Request-ID': 'cy-postal-context-test',
    },
    body: JSON.stringify({
      countryCode: 'CY',
      ...CYPRUS_POSTAL_CONTEXT_TEST_POINT,
  ...
  assert.equal(body.data.countryCode, 'CY');
...
test('Cyprus postcode route accepts the CY- prefix and gates geometry', async () => {
  const query = new URLSearchParams({
    validAt: CYPRUS_POSTAL_CONTEXT_TEST_INSTANT,
    geometry: 'geojson',
  });
  const response = await fetch(`${baseUrl}/api/postal/CY/CY-0000?${query}`, {
    headers: { 'X-AGID-Request-ID': 'cy-postal-lookup-test' },
  });
  const body = await response.json() as any;
