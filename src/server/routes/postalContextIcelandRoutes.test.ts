...
test('Iceland Postal Context route resolves source-linked building context with AGID', async () => {
  const response = await fetch(`${baseUrl}/api/postal/resolve`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-AGID-Request-ID': 'is-postal-context-test',
    },
    body: JSON.stringify({
      countryCode: 'IS',
      ...ICELAND_POSTAL_CONTEXT_TEST_POINT,
  ...
  assert.equal(body.data.countryCode, 'IS');
...
test('Iceland postcode route preserves three digits and gates geometry', async () => {
  const query = new URLSearchParams({
    validAt: ICELAND_POSTAL_CONTEXT_TEST_INSTANT,
    geometry: 'geojson',
  });
  const response = await fetch(`${baseUrl}/api/postal/IS/000?${query}`, {
    headers: { 'X-AGID-Request-ID': 'is-postal-lookup-test' },
  });
  const body = await response.json() as any;
