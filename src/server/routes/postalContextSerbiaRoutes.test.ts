...
test('Serbia Postal Context route resolves an explicitly linked RGZ building with RS AGID', async () => {
  const response = await fetch(`${baseUrl}/api/postal/resolve`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-AGID-Request-ID': 'rs-postal-context-test',
    },
    body: JSON.stringify({
      countryCode: 'RS',
      ...SERBIA_POSTAL_CONTEXT_TEST_POINT,
  ...
  assert.equal(body.data.countryCode, 'RS');
...
test('Serbia postcode route canonicalizes whitespace and gates derived geometry', async () => {
  const query = new URLSearchParams({
    validAt: SERBIA_POSTAL_CONTEXT_TEST_INSTANT,
    geometry: 'geojson',
  });
  const response = await fetch(`${baseUrl}/api/postal/RS/${encodeURIComponent('00 000')}?${query}`, {
    headers: { 'X-AGID-Request-ID': 'rs-postal-lookup-test' },
  });
  const body = await response.json() as any;
