...
const validAt = '2026-08-31T18:08:30.995Z';
let server: Server;
let baseUrl: string;

async function lookup(postalCode: string, expectedStatus = 200) {
  const response = await fetch(`${baseUrl}/api/postal/CO/${encodeURIComponent(postalCode)}?validAt=${encodeURIComponent(validAt)}&geometry=geojson`);
...
before(async () => {
  const loaded = loadPostalContextPack(
    resolve('data/postal_country_packs/co/postal-context/m2/descriptor.json'),
    descriptorDigest,
...
test('real CO lookup reaches API, app conversion, fit, translucent paint, clear and re-search', async () => {
  const data = await lookup('１１０ ９１１');
  assert.equal(data.status, 'unique');
  assert.equal(data.normalizedPostalCode, '110911');
  ...
  assert.equal(data.geometries[0].source.sourceDate, '2016-06-28');
  ...
  assert.deepEqual(postalAreaBounds(collection), [[-74.177047, 4.666119], [-74.118246, 4.717995]]);
...
test('CO invalid input is rejected without a fabricated surface', async () => {
  const response = await fetch(`${baseUrl}/api/postal/CO/110-911?validAt=${encodeURIComponent(validAt)}&geometry=geojson`);
