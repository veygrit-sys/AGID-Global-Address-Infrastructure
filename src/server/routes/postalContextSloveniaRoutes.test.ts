...
test('Slovenia Postal Context route resolves an explicitly linked GURS building with SI AGID', async () => {
  const response = await fetch(`${baseUrl}/api/postal/resolve`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-AGID-Request-ID': 'si-postal-context-test',
    },
    body: JSON.stringify({
      countryCode: 'SI',
      ...SLOVENIA_POSTAL_CONTEXT_TEST_POINT,
  ...
  assert.equal(body.data.countryCode, 'SI');
...
test('Slovenia postcode route canonicalizes SI- input and gates postal-district geometry', async () => {
  const query = new URLSearchParams({
    validAt: SLOVENIA_POSTAL_CONTEXT_TEST_INSTANT,
    geometry: 'geojson',
  });
  const response = await fetch(`${baseUrl}/api/postal/SI/SI-0000?${query}`, {
    headers: { 'X-AGID-Request-ID': 'si-postal-lookup-test' },
  });
  const body = await response.json() as any;
