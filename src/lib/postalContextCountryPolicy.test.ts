...
import {
  ...
  normalizeSaintVincentGrenadinesPostalCode,
...
test('normalizes supported country postal codes without cross-country guessing', () => {
  ...
  assert.equal(normalizeSaintVincentGrenadinesPostalCode('ｖｃ０１２０'), 'VC0120');
  assert.equal(normalizeSaintVincentGrenadinesPostalCode('vc 0120'), 'VC0120');
  assert.equal(normalizeSaintVincentGrenadinesPostalCode('VC-0120'), null);
  assert.equal(normalizeSaintVincentGrenadinesPostalCode('0120'), null);
  assert.equal(normalizeSaintVincentGrenadinesPostalCode('VC120'), null);
  assert.equal(normalizePostalContextPostalCode('vc', 'ｖｃ ０１２０'), 'VC0120');
  ...
  assert.equal(normalizeParaguayPostalCode('００１５１８'), '001518');
  assert.equal(normalizeParaguayPostalCode('001 518'), '001518');
  assert.equal(normalizeParaguayPostalCode('001-518'), null);
  assert.equal(normalizeParaguayPostalCode('PY-001518'), null);
  assert.equal(normalizeParaguayPostalCode('01518'), null);
  assert.equal(normalizePostalContextPostalCode('py', '001 518'), '001518');
  ...
  assert.equal(normalizeFalklandIslandsPostalCode('ＦＩＱＱ １ＺＺ'), 'FIQQ 1ZZ');
  assert.equal(normalizeFalklandIslandsPostalCode('fiqq1zz'), 'FIQQ 1ZZ');
...
test('declares country-specific full-code geometry semantics', () => {
  ...
  assert.equal(isPostalContextCountryCode('VC'), true);
  assert.equal(POSTAL_CONTEXT_COUNTRY_POLICIES.VC.fullCodeGeometrySemantics, 'area-or-non-area');
  assert.equal(POSTAL_CONTEXT_COUNTRY_POLICIES.VC.postalCodeFormat, 'VCNNNN');
  ...
  assert.equal(isPostalContextCountryCode('FK'), true);
  assert.equal(POSTAL_CONTEXT_COUNTRY_POLICIES.FK.fullCodeGeometrySemantics, 'postal-area-first');
  assert.equal(POSTAL_CONTEXT_COUNTRY_POLICIES.FK.postalCodeFormat, 'FIQQ 1ZZ (single postcode for the whole territory)');
  ...
  assert.equal(isPostalContextCountryCode('MZ'), true);
  assert.equal(POSTAL_CONTEXT_COUNTRY_POLICIES.MZ.fullCodeGeometrySemantics, 'postal-area-first');
  assert.match(POSTAL_CONTEXT_COUNTRY_POLICIES.MZ.postalCodeFormat, /NNNNN-NNN.*74\/2024.*eight-digit.*territorial.*urban.*capital.*legacy four-digit.*NNNN-NN.*not current.*assignment.*geometry.*separate/i);
