...
const descriptorPath = resolve('data/postal_country_packs/ac/postal-context/m2/descriptor.json');
const geometryPath = resolve('data/postal_country_packs/ac/postal-context/m2/geometry.json');
...
test('AC descriptor loads through the shared runtime with one real postal node and two surfaces', () => {
  const descriptorDigest = digest(descriptorPath);
  assert.equal(descriptorDigest, 'sha256:6b0e08e1c8e461e7fee847cc06ea13e5f0f8abc1cb9a5a2ade8bd258c67537e1');
  const loaded = loadPostalContextPack(descriptorPath, descriptorDigest, {
    expectedCountryCode: 'AC', allowExperimental: true,
  });
  assert.equal(loaded.descriptor.countryCode, 'AC');
  assert.equal(loaded.descriptor.releaseId, 'ac-upu-geoboundaries-20260903');
...
test('AC output preserves all selected source positions in valid derived MultiPolygons', () => {
  ...
  assert.equal(topology.positionCount, 1261);
  ...
  assert.ok(geometry.features.every(item => item.nodeId === 'postal-ac-ascn-1zz'));
  ...
  assert.deepEqual(combinedBounds, [-14.420263125670147, -7.992611073283626, -14.294916449140885, -7.88966415023782]);
  assert.ok(Math.abs(totalArea / 1e6 - 96.58210569103102) < 1e-9);
});
