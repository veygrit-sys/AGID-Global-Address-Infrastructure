...
const seedRoot = resolve(root, 'data/postal_country_packs/al/postal-context');

function readJson<T>(name: string): T {
  return JSON.parse(readFileSync(resolve(seedRoot, name), 'utf8')) as T;
}

test('Albania seed separates postal assignment, derived surfaces, addresses, and buildings', () => {
  const manifest = readJson<RepositoryManifest>('repository-manifest.json');

  assert.equal(manifest.repository.name, 'agid-postal-al');
  assert.equal(manifest.repository.country_code, 'AL');
  ...
  assert.equal(manifest.postal_system.full_code_name, 'Kodi postar');
  assert.equal(manifest.postal_system.full_code_format, 'NNNN');
  assert.equal(manifest.postal_system.full_code_default_geometry, 'delivery_network_assignment_with_optional_derived_surface');
  assert.match(manifest.postal_system.perimeter_rule, /derived.*rights-cleared.*explicitly derived/i);
  assert.match(manifest.postal_system.address_rule, /National Address System.*bulk redistribution license/i);
  assert.match(manifest.postal_system.building_rule, /official address or cadastral object identifier.*proximity.*candidates/i);
  assert.match(manifest.postal_system.territory_rule, /Albania.*Kosovo.*separate country packs/i);
  assert.equal(manifest.promotion.current_stage, 'M1_metadata');
  assert.ok(manifest.promotion.hard_blockers.includes('post-office-or-branch-point-presented-as-postcode-area'));
  ...
  assert.ok(manifest.promotion.hard_blockers.includes('kosovo-or-neighboring-feature-published-with-al-identity'));
  assert.ok(manifest.promotion.hard_blockers.includes('postcode-stored-as-number'));
});

test('Albania source policy separates postal, address-system, cadastral, and geoportal authority', () => {
  const profile = readJson<SourceProfile>('source-profile.json');
  const sources = new Map(profile.sources.map(source => [source.source_id, source]));
  const postal = sources.get('posta-shqiptare-postcodes');
  const upu = sources.get('upu-albania-addressing');
  const address = sources.get('albania-national-address-system');
  const cadastre = sources.get('ashk-albania-cadastral-buildings');
  const geoportal = sources.get('asig-albania');
  ...
  assert.ok(postal?.prohibited_claims.includes('office-point-is-postcode-area'));
  assert.equal(upu?.geometry_authority, 'none');
  assert.ok(upu?.prohibited_claims.includes('format-proves-current-allocation'));
  assert.equal(address?.assignment_authority, 'official_national_address_authority');
  ...
  assert.equal(geoportal?.redistribution_class, 'R4_validation_only');
  assert.ok(geoportal?.prohibited_claims.includes('portal-implies-one-license'));
  assert.ok(profile.artifact_partitions.some(partition => partition.id === 'derived-postal-surface'));
  assert.ok(profile.artifact_partitions.some(partition => partition.id === 'coverage-and-country'));
  assert.ok(profile.artifact_partitions.some(partition => partition.id === 'private-and-sensitive'));
});

test('Albania fixtures use synthetic 000x strings and never become production evidence', () => {
  const pack = readJson<SyntheticFixturePack>('fixtures/albania-synthetic.json');
  ...
  assert.equal(pack.country_code, 'AL');
  ...
  assert.ok(pack.fixtures.every(fixture => fixture.fixture_id.startsWith('al-syn-')));
  ...
  assert.ok(prohibited.has('office-point-is-postcode-area'));
  assert.ok(prohibited.has('kosovo-is-albania'));
