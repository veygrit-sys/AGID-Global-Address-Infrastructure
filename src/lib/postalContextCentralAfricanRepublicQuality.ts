export type CentralAfricanRepublicPostalCandidate = {
  value?: string | null;
  objectKind: 'postcode' | 'po-box' | 'private-bag' | 'locality' | 'post-office' | 'route' | 'administrative-area' | 'agid-cell' | 'model-output' | string;
  geometryType?: 'Point' | 'LineString' | 'Polygon' | 'MultiPolygon' | string | null;
  sourceKind: 'official-reference' | 'open-reference' | 'model' | string;
};

export function assessCentralAfricanRepublicPostalCandidate(candidate: CentralAfricanRepublicPostalCandidate) {
