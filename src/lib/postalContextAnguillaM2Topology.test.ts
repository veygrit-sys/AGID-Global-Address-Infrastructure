...
const descriptorPath = resolve('data/postal_country_packs/ai/postal-context/m2/descriptor.json');
const geometryPath = resolve('data/postal_country_packs/ai/postal-context/m2/geometry.json');
...
test('AI descriptor loads through the shared runtime with one real postal node and two surfaces', () => {
  const descriptorDigest = digest(descriptorPath);
  assert.equal(descriptorDigest, 'sha256:19afd7f8455a9314d179f05e4ebd29cdfa619287daf314fa15294efbfd99b6c5');
  const loaded = loadPostalContextPack(descriptorPath, descriptorDigest, {
    expectedCountryCode: 'AI', allowExperimental: true,
  });
  assert.equal(loaded.descriptor.countryCode, 'AI');
  assert.equal(loaded.descriptor.releaseId, 'ai-upu-geoboundaries-20260831');
...
test('AI output preserves all source positions in two independently valid derived MultiPolygons', () => {
  ...
  assert.equal(topology.positionCount, 10606);
  ...
  assert.ok(geometry.features.every(item => item.nodeId === 'postal-ai-ai-2640'));
  ...
  assert.deepEqual(combinedBounds, [-63.42901273399514, 18.155156419596608, -62.926366790615475, 18.595112128484345]);
  assert.ok(Math.abs(totalArea / 1e6 - 74.86260878930202) < 1e-9);
});
