...
const seedRoot = resolve(root, 'data/postal_country_packs/lv/postal-context');

function readJson<T>(name: string): T {
  return JSON.parse(readFileSync(resolve(seedRoot, name), 'utf8')) as T;
}

test('Latvia seed separates postal membership, civic addresses, derived surfaces, and buildings', () => {
  const manifest = readJson<RepositoryManifest>('repository-manifest.json');

  assert.equal(manifest.repository.name, 'agid-postal-lv');
  assert.equal(manifest.repository.country_code, 'LV');
  ...
  assert.equal(manifest.postal_system.full_code_name, 'pasta indekss');
  assert.equal(manifest.postal_system.full_code_format, 'LV-NNNN');
  assert.equal(manifest.postal_system.full_code_default_geometry, 'address_range_locality_or_non_area');
  assert.match(manifest.postal_system.assignment_rule, /house-number.*not an official polygon/i);
  assert.match(manifest.postal_system.special_code_rule, /organization.*non-areal/i);
  assert.match(manifest.postal_system.address_rule, /VZD.*does not replace Latvijas Pasts/i);
  assert.match(manifest.postal_system.postal_surface_rule, /complete address-membership.*derived.*noncanonical/i);
  assert.match(manifest.postal_system.building_rule, /explicit.*identifier.*nearest.*candidate/i);
  assert.match(manifest.postal_system.administrative_rule, /cannot create.*postcode/i);
  assert.match(manifest.postal_system.territory_rule, /Estonia.*Lithuania.*Russia.*Belarus/i);
  assert.match(manifest.temporal_model.history_rule, /never rewrites/i);
  assert.equal(manifest.promotion.current_stage, 'M1_metadata');
  assert.ok(manifest.promotion.hard_blockers.includes('address-list-presented-as-official-postcode-boundary'));
  assert.ok(manifest.promotion.hard_blockers.includes('nearest-building-presented-as-exact-link'));
  assert.ok(manifest.promotion.hard_blockers.includes('restricted-security-object-published'));
});

test('Latvia source policy keeps Latvijas Pasts and VZD authority roles distinct', () => {
  const profile = readJson<SourceProfile>('source-profile.json');
  const sources = new Map(profile.sources.map(source => [source.source_id, source]));
  const post = sources.get('latvijas-pasts-check-address');
  const addresses = sources.get('vzd-latvia-address-register');
  const buildings = sources.get('vzd-latvia-cadastral-buildings');
  const boundaries = sources.get('vzd-latvia-administrative-boundaries');
  ...
  assert.ok(addresses?.prohibited_claims.includes('postal-operator-validation-from-vzd-alone'));
  ...
  assert.ok(profile.artifact_partitions.some(partition => partition.id === 'special-and-endpoint'));
...
test('Latvia fixtures are synthetic and never prove real assignment or geometry', () => {
  const pack = readJson<SyntheticFixturePack>('fixtures/latvia-synthetic.json');
  ...
  assert.equal(pack.country_code, 'LV');
  ...
  assert.equal(pack.fixture_policy.postal_prefix, 'LV-000');
  assert.ok(postalCodes.every(code => /^LV-\d{4}$/.test(code)));
  assert.ok(pack.fixtures.every(fixture => fixture.fixture_id.startsWith('lv-syn-')));
  ...
  assert.ok(prohibited.has('official-latvijas-pasts-boundary'));
  assert.ok(prohibited.has('address-is-mail-deliverable'));
  assert.ok(prohibited.has('nearest-building-is-exact-link'));
  assert.ok(prohibited.has('organization-code-is-residential-area'));
});
