...
const seedRoot = resolve(root, 'data/postal_country_packs/am/postal-context');

function readJson<T>(name: string): T {
  return JSON.parse(readFileSync(resolve(seedRoot, name), 'utf8')) as T;
}

test('Armenia seed separates postal assignment, derived surfaces, addresses, and buildings', () => {
  const manifest = readJson<RepositoryManifest>('repository-manifest.json');

  assert.equal(manifest.repository.name, 'agid-postal-am');
  assert.equal(manifest.repository.country_code, 'AM');
  ...
  assert.match(manifest.postal_system.full_code_default_geometry, /postal_region.*post_office.*derived_surface/);
  assert.match(manifest.postal_system.perimeter_rule, /derived.*rights-cleared.*explicitly derived/i);
  assert.match(manifest.postal_system.address_rule, /Cadastre Committee.*community.*bulk redistribution license/i);
  ...
  assert.ok(manifest.promotion.hard_blockers.includes('postal-region-or-post-office-point-presented-as-postcode-area'));
  ...
  assert.ok(manifest.promotion.hard_blockers.includes('disputed-feature-auto-assigned-to-am'));
  assert.ok(manifest.promotion.hard_blockers.includes('postcode-stored-as-number'));
});

test('Armenia source policy separates postal, address-register, building, and map authority', () => {
  const profile = readJson<SourceProfile>('source-profile.json');
  const sources = new Map(profile.sources.map(source => [source.source_id, source]));
  const postal = sources.get('haypost-am');
  const upu = sources.get('upu-armenia-addressing');
  const address = sources.get('armenia-real-estate-address-register');
  const building = sources.get('armenia-national-geoportal-buildings');
  const cadastre = sources.get('cadastre-armenia');
  ...
  assert.equal(address?.assignment_authority, 'official_real_estate_address_register');
  assert.ok(address?.prohibited_claims.includes('address-point-is-building-footprint'));
  assert.equal(building?.geometry_authority, 'official_building_or_cadastral_geometry_when_published');
  ...
  assert.ok(cadastre?.prohibited_claims.includes('portal-implies-one-license'));
...
test('Armenia fixtures use synthetic 000x strings and never become production evidence', () => {
  const pack = readJson<SyntheticFixturePack>('fixtures/armenia-synthetic.json');
  ...
  assert.equal(pack.country_code, 'AM');
  ...
  assert.ok(pack.fixtures.every(fixture => fixture.fixture_id.startsWith('am-syn-')));
  ...
  assert.ok(prohibited.has('disputed-feature-auto-assigned-to-am'));
