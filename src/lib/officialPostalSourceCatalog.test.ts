...
test('prefers country-specific official sources before the UPU global fallback', () => {
  const countrySpecificCountries = ['AO', 'BI', 'BJ', 'DJ', 'DZ', 'EG', 'ET', 'GH', 'KE', 'LR', 'MA', 'MW', 'MZ', 'NA', 'NG', 'SC', 'SO', 'SS', 'TN', 'TZ', 'UG', 'RW', 'ZM', 'ZW', 'MG', 'MU', 'BW', 'AT', 'CH', 'DE', 'CZ', 'DK', 'MT', 'MC', 'FI', 'BG', 'BY', 'BE', 'ME', 'TW', 'LV', 'LT', 'LI', 'NL', 'PT', 'JE', 'IM', 'GI'];
  ...
  const benin = getOfficialPostalSourcesForCountry('BJ').find(source => source.id === 'la-poste-benin');
  assert.equal(benin?.sourceRole, 'context-only');
  assert.equal(benin?.validationReadiness, 'metadata-only');
