...
> = {
  ...
  TC: {
    countryCode: 'TC',
    postalCodeFormat: 'TKCA 1ZZ (single postcode for the whole territory)',
    fullCodeGeometrySemantics: 'postal-area-first',
  },
  AI: {
    countryCode: 'AI',
  ...
  AC: {
    countryCode: 'AC',
    postalCodeFormat: 'ASCN 1ZZ (single postcode for the whole territory)',
    fullCodeGeometrySemantics: 'postal-area-first',
  },
  NI: {
    countryCode: 'NI',
...
export function normalizeSaintVincentGrenadinesPostalCode(value: unknown) {
  const normalized = String(value ?? '')
    .normalize('NFKC')
    .toUpperCase()
    .replace(/\s+/g, '');
  return /^VC\d{4}$/.test(normalized) ? normalized : null;
}
...
export function normalizeTurksCaicosPostalCode(value: unknown) {
  const normalized = String(value ?? '')
    .normalize('NFKC')
    .toUpperCase()
    .replace(/\s+/g, '');
  return normalized === 'TKCA1ZZ' ? 'TKCA 1ZZ' : null;
}
...
export function normalizeAscensionIslandPostalCode(value: unknown) {
  const normalized = String(value ?? '')
    .normalize('NFKC')
    .toUpperCase()
    .replace(/\s+/g, '');
  return normalized === 'ASCN1ZZ' ? 'ASCN 1ZZ' : null;
}
...
export function normalizePostalContextPostalCode(countryCode: string, value: unknown) {
  ...
  if (normalizedCountry === 'VC') return normalizeSaintVincentGrenadinesPostalCode(value);
  ...
  if (normalizedCountry === 'TC') return normalizeTurksCaicosPostalCode(value);
  ...
  if (normalizedCountry === 'AC') return normalizeAscensionIslandPostalCode(value);
